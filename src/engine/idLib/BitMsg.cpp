...
bool idBitMsg::CheckOverflow( int numBits ) {
	...
	if ( numBits > GetRemainingWriteBits() ) {
		if ( !allowOverflow ) {
			Com_FatalError( "idBitMsg: overflow without allowOverflow set" );
		}
		if ( numBits > ( maxSize << 3 ) ) {
			Com_FatalError( "idBitMsg: %i bits is > full message size", numBits );
		}
		Com_Printf( "idBitMsg: overflow\n" );
		BeginWriting();
		overflowed = true;
		return true;
	}
...
byte *idBitMsg::GetByteSpace( int length ) {
	...
	if ( !writeData ) {
		Com_FatalError( "idBitMsg::GetByteSpace: cannot write to message" );
	}
...
void idBitMsg::WriteBits( int value, int numBits ) {
	...
	if ( !writeData ) {
		Com_DropError( "idBitMsg::WriteBits: cannot write to message" );
	}

	// check if the number of bits is valid
	if ( numBits == 0 || numBits < -31 || numBits > 32 ) {
		Com_DropError( "idBitMsg::WriteBits: bad numBits %i", numBits );
	}
	...
	if ( numBits != 32 ) {
		if ( numBits > 0 ) {
			if ( value > ( 1 << numBits ) - 1 ) {
				Com_Warning( "idBitMsg::WriteBits: value overflow %d %d", value, numBits );
			} else if ( value < 0 ) {
				Com_Warning( "idBitMsg::WriteBits: value overflow %d %d", value, numBits );
			}
		} else {
...
int idBitMsg::ReadBits( int numBits ) const {
	...
	if ( !readData ) {
		Com_FatalError( "idBitMsg::ReadBits: cannot read from message" );
	}

	// check if the number of bits is valid
	if ( numBits == 0 || numBits < -31 || numBits > 32 ) {
		Com_FatalError( "idBitMsg::ReadBits: bad numBits %i", numBits );
	}
