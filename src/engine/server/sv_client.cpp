...
void SV_DirectConnect( netadr_t from ) {
	char				userinfo[MAX_INFO_STRING], *password, *denied, *ip;
	int					i, clientNum, qport, challenge, startIndex, count, oldInfoLen2 = strlen(userinfo), newInfoLen2;
	client_t			*cl, *newcl;
...
void SV_WriteDownloadToClient(client_t * cl, msg_t * msg) {
	int             curindex,rate, blockspersnap, idPack, download_flag;
...
static void SV_VerifyPaks_f(client_t * cl)  {
	int             nChkSum1, nChkSum2, nClientPaks, nServerPaks, i, j, nCurArg, nClientChkSum[1024], nServerChkSum[1024];
	...
	if(sv_pure->integer != 0) {
		...
		bGood = (qboolean)( FS_FileIsInPAK( Sys_GetDLLName("cgame"), &nChkSum1 ) == 1 );
		if ( bGood ) {
			bGood = (qboolean)( FS_FileIsInPAK( Sys_GetDLLName("ui"), &nChkSum2 ) == 1 );
		}
...
void SV_ExecuteClientCommand(client_t * cl, const char *s, qboolean clientOK, qboolean premaprestart) {
	...
	int				argsFromOneMaxlen, charCount, dollarCount, i;
	char			*arg;
	...
	if(clientOK) {
		...
		if (!u->name && sv.state == SS_GAME) {
			...
			if (argsFromOneMaxlen >= 0) {
				...
				Com_Printf("Callvote from %s (client #%i, %s): %s\n", cl->name, (int) (cl - svs.clients), NET_AdrToString(cl->netchan.remoteAddress), Cmd_Args());
			}
...
static qboolean SV_ClientCommand(client_t * cl, msg_t * msg, qboolean premaprestart) {
	int             seq;
	const char     *s;
	qboolean        clientOk = qtrue, floodprotect = qtrue;
...
static void SV_UserVoip(client_t *cl, msg_t *msg) {
	int					sender, generation, sequence, frames, packetsize, flags, i;
