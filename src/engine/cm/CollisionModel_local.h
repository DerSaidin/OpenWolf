...
typedef struct cm_traceWork_s {
	...
	d3trace_t trace;									// collision detection result
...
public:
	...
	void			LoadMap( const char *fname, bool clientload, int *checksum );
	...
	cmHandle_t		SetupTrmModel( const idTraceModel &trm, idMaterial *material );
	...
	void			Translation( d3trace_t *results, const idVec3 &start, const idVec3 &end,
								const idTraceModel *trm, const idMat3 &trmAxis, int contentMask,
								cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
	// rotates a trm and reports the first collision if any
	void			Rotation( d3trace_t *results, const idVec3 &start, const idRotation &rotation,
								const idTraceModel *trm, const idMat3 &trmAxis, int contentMask,
								cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
...
private:			// CollisionMap_rotate.cpp
	...
	void			Rotation180( d3trace_t *results, const idVec3 &rorg, const idVec3 &axis,
...
public:			// CollisionMap_contents.cpp
	...
	int				TransformedPointContents( const idVec3 &p, cmHandle_t model, const idVec3 &origin, const idVec3 &modelAngles ) {
		idMat3 axis;
		//axis = modelAngles.ToMat3()
		AnglesToAxis(modelAngles.getPtr(),axis.getAxis());
		return TransformedPointContents(p,model,origin,axis);
	}
private:
	...
	int				ContentsTrm( d3trace_t *results, const idVec3 &start,
									const idTraceModel *trm, const idMat3 &trmAxis, int contentMask,
									cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis );
...
private:			// CollisionMap_trace.cpp
	...
	void			RecurseProcBSP_r( d3trace_t *results, int parentNodeNum, int nodeNum, float p1f, float p2f, const idVec3 &p1, const idVec3 &p2 );
...
private:			// CollisionMap_load.cpp
	...
	void			LoadProcBSP( const char *name );
					// loading of Q3 BSP tree
	void			ParseBSPNodes( dnode_t *nodes, int numNodes, dplane_t *planes );
	...
	void			CreatePolygon( cm_model_t *model, idFixedWinding *w, const idPlane &plane, idMaterial *material, int primitiveNum );
	void			PolygonFromWinding( cm_model_t *model, idFixedWinding *w, const idPlane &plane, idMaterial *material, int primitiveNum );
	void			CalculateEdgeNormals( cm_model_t *model, cm_node_t *node );
	void			CreatePatchPolygons( cm_model_t *model, idSurface_Patch &mesh, idMaterial *material, int primitiveNum );
	...
	void			BuildModelsBSP( const char *fname );
	...
	cm_model_t *	CollisionModelForBSPSubModel(  int index, dmodel_t *m, dbrush_t *brushes, 
		dbrushside_t *brushSides, dplane_t *planes, drawVert_t *verts, dsurface_t *surfs, int *indexes); // Q3 bsp submodels
...
private:			// CollisionMap_files.cpp
					// writing
	void			WriteNodes( fileHandle_t, cm_node_t *node );
	int				CountPolygonMemory( cm_node_t *node ) const;
	void			WritePolygons( fileHandle_t, cm_node_t *node );
	int				CountBrushMemory( cm_node_t *node ) const;
	void			WriteBrushes( fileHandle_t, cm_node_t *node );
	void			WriteCollisionModel( fileHandle_t, cm_model_t *model );
...
private:			// CollisionMap_debug
	...
	idMaterial *declManager_FindMaterial(const char *name, bool smth = false);
