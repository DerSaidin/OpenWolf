    ...
    if ( inwater ){
        ...
        if(!num_drops)
        {
            ...
            for( i = 0; i < z; i ++)
            {
                ...
                dropShader[num_drops] = trap_R_RegisterShader(va("hud/drop%i", ((int) ( 2 + 2 * crandom ()))));
...
static void CG_UnderwaterSounds(void) {
	trap_S_AddLoopingSound( cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin, cgs.media.underWaterSound);
}
