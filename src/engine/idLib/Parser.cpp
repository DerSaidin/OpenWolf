...
void idParser::PrintDefine( define_t *define ) {
	Com_Printf("define->name = %s\n", define->name);
	Com_Printf("define->flags = %d\n", define->flags);
	Com_Printf("define->builtin = %d\n", define->builtin);
	Com_Printf("define->numparms = %d\n", define->numparms);
}
...
int idParser::ReadSourceToken( idToken *token ) {
	...
	if ( !idParser::scriptstack ) {
		Com_FatalError( "idParser::ReadSourceToken: not loaded" );
		return false;
	}
	changedScript = 0;
...
int PC_idLibOperatorPriority(int op) {
	switch(op) {
		...
		switch( t->type ) {
			...
			case TT_PUNCTUATION:
			{
				...
				if (!error && !negativevalue) {
					...
					AllocOperator(o);
					o->op = t->subtype;
					o->priority = PC_idLibOperatorPriority(t->subtype);
...
int idParser::LoadFile( const char *filename, bool OSPath ) {
	...
	if ( idParser::loaded ) {
		Com_FatalError("idParser::loadFile: another source already loaded");
		return false;
	}
...
int idParser::LoadMemory(const char *ptr, int length, const char *name ) {
	...
	if ( idParser::loaded ) {
		Com_FatalError("idParser::loadMemory: another source already loaded");
		return false;
	}
