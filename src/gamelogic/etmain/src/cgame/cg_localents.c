...
void CG_AddFragment(localEntity_t * le)
{
	...
	if(le->onFireStart && (le->onFireStart < cg.time && le->onFireEnd > cg.time))
	{
		...
		trap_S_AddLoopingSound(-1, le->refEntity.origin, vec3_origin, cgs.media.flameCrackSound);
	}
...
//----(SA)  end
	...
	if(hasFlame)
	{
		...
		if(VectorLengthSquared(flameDir) == 0)
		{
			...
			trap_S_AddLoopingSound(0, newOrigin, vec3_origin, cgs.media.flameSound);
		}
		else
		{
			VectorNormalize(flameDir);
			// play a flame blow sound when moving
			trap_S_AddLoopingSound(0 ,newOrigin, vec3_origin, cgs.media.flameBlowSound);
		}
	}
