...
static qboolean IRC_Parser( char next ) {
	...
	return (qboolean)(has_msg && !IRC_ParserError);
}
...
static void IRC_Display( int event , const char * nick , const char *message ) {
	...
	switch ( IRC_EventType( event ) ) {
		...
		case IRC_EVT_JOIN:
			has_message = qfalse;
			has_nick = (qboolean)!IRC_EventIsSelf( event );
			if ( has_nick ) {
		...
		case IRC_EVT_PART:
			...
			// someone else.
			has_nick = qtrue;
			has_message = (qboolean)( message[0] != 0 );
			if ( has_message ) {
...
static int IRCH_PrivMsg( )
{
	...
	is_channel = (qboolean)(IRC_String( arg_values[ 0 ] )[ 0 ] == '#');
...
static qboolean IRC_ProcessSendQueue() {
	...
	return (qboolean)( rv == IRC_CMD_SUCCESS );
}
...
static qboolean IRC_InitialiseUser( const char * name ) {
	...
	ovrnn = (qboolean)(cl_IRC_override_nickname->integer && strlen( cl_IRC_nickname->string ));
	...
	return (qboolean)( IRC_User.nicklen > 0 && replaced < IRC_User.nicklen / 2 );
}
...
static qboolean IRC_InitialConnect( ) {
	...
	return (qboolean)( err_code == IRC_CMD_SUCCESS );
}
...
qboolean CL_OW_IRCIsConnected(void) {
	// get IRC status
	return (qboolean)( IRC_ThreadStatus == IRC_THREAD_JOINED );
}
...
qboolean CL_OW_IRCIsRunning(void) {
	// return IRC status
	return (qboolean)( IRC_ThreadStatus != IRC_THREAD_DEAD );
}
