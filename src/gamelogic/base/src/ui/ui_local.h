...
typedef enum
{
	UI_OW_HIGHSCORE_RESPONSE,		//  response to a highscore request
	UI_OW_REPORT_HIGHSCORE_RESPONSE,//  reponse to a highscore report
	UI_OW_AUTHORIZED,				//  we got a response to our authorize request
	UI_OW_REQUEST_HIGHSCORES,		// request highscores from the master server
	UI_OW_SERVER_ERRORMESSAGE,		// server got a error message it wants us to display
} OpenWolfCmd_t;

extern int QDECL	UI_OW_exec( int cmd, ... );
