...
qboolean Load_Menu(int handle) {
  ...
  while ( 1 ) {
	...
	if(cl_lang) {
		...
		// Dushan - if cl_language is 3
		//        - load Italian
		else if(cl_lang == 3)
		{
			s = va("%s%s", out, "italian/");
		}
		// Dushan - if cl_language is 4
		//        - load Spanish
		else if(cl_lang == 4)
		{
...
static void UI_OW_PlayerSelectSlot_SetModel( playerInfo_t *info, char * model )
{
	vec3_t	viewangles;
	vec3_t	moveangles;
	...
	UI_PlayerInfo_SetInfo( info, LEGS_IDLE, LEGS_IDLE, viewangles, vec3_origin, WP_BLASTER, qfalse );
}
					...
					case AUTHORIZE_ACCOUNTINFO:
						{
							int i;
							for ( i=0; i<MAX_CHARACTERS; i++ ) {
								char * s = Info_ValueForKey( info, va("%d", i ) );

								if ( s && s[0] ) {
									characterInfo_t * c = uiInfo.characters + i;

									Q_strncpyz( c->name, s, sizeof(c->name) );
									Q_strncpyz( c->model, s, sizeof(c->model) );
								
									c->rank = atoi(COM_Parse( &s ));

									UI_OW_PlayerSelectSlot_SetModel( &c->info, c->model );
								} else
