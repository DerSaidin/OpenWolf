...
static void CG_DrawFlashFire(void)
{
	...
	if(alpha > 0)
	{
		...
		trap_S_AddLoopingSound(cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin, cgs.media.flameSound);
		trap_S_AddLoopingSound(cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin, cgs.media.flameCrackSound);
	}
	else
	{
		cg.v_noFireTime = cg.time;
