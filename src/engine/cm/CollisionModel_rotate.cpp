...
================
*/
void idCollisionModelManagerLocal::Rotation180( d3trace_t *results, const idVec3 &rorg, const idVec3 &axis,
	...
	if ( model < 0 || model > MAX_SUBMODELS || model > idCollisionModelManagerLocal::maxModels ) {
		Com_Printf("idCollisionModelManagerLocal::Rotation180: invalid model handle\n");
		return;
	}
	if ( !idCollisionModelManagerLocal::models[model] ) {
		Com_Printf("idCollisionModelManagerLocal::Rotation180: invalid model\n");
		return;
	}
...
void idCollisionModelManagerLocal::Rotation( d3trace_t *results, const idVec3 &start, const idRotation &rotation,
										const idTraceModel *trm, const idMat3 &trmAxis, int contentMask,
										cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis ) {
	idVec3 tmp;
