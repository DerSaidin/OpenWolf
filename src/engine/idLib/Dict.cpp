...
void idDict::TransferKeyValues( idDict &other ) {
	...
	if ( other.args.Num() && other.args[0].key->GetPool() != &globalKeys ) {
		Com_FatalError( "idDict::TransferKeyValues: can't transfer values across a DLL boundary" );
		return;
	}
...
static void WriteString( const char *s, idFile *f ) {
	...
	if ( len >= MAX_STRING_CHARS-1 ) {
		Com_DropError( "idDict::WriteToFileHandle: bad string" );
	}
...
const idKeyValue *idDict::FindKey( const char *key ) const {
	...
	if ( key == NULL || key[0] == '\0' ) {
		Com_DPrintf( "idDict::FindKey: empty key" );
		return NULL;
	}
...
int idDict::FindKeyIndex( const char *key ) const {
	...
	if ( key == NULL || key[0] == '\0' ) {
		Com_DPrintf( "idDict::FindKeyIndex: empty key" );
		return NULL;
	}
...
static idStr ReadString( idFile *f ) {
	...
	if ( len == MAX_STRING_CHARS ) {
		Com_DropError( "idDict::ReadFromFileHandle: bad string" );
	}
