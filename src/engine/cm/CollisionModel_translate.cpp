...
ID_INLINE void CM_AddContact( cm_traceWork_t *tw ) {
	...
	// copy contact information from d3trace_t
...
void idCollisionModelManagerLocal::Translation( d3trace_t *results, const idVec3 &start, const idVec3 &end,
	...
	assert( ((byte *)&start) < ((byte *)results) || ((byte *)&start) >= (((byte *)results) + sizeof( d3trace_t )) );
	assert( ((byte *)&end) < ((byte *)results) || ((byte *)&end) >= (((byte *)results) + sizeof( d3trace_t )) );
	assert( ((byte *)&trmAxis) < ((byte *)results) || ((byte *)&trmAxis) >= (((byte *)results) + sizeof( d3trace_t )) );

	memset( results, 0, sizeof( *results ) );

	if ( model < 0 || model > MAX_SUBMODELS || model > idCollisionModelManagerLocal::maxModels ) {
		Com_Printf("idCollisionModelManagerLocal::Translation: invalid model handle\n");
		return;
	}
	if ( !idCollisionModelManagerLocal::models[model] ) {
		//Com_Printf("idCollisionModelManagerLocal::Translation: invalid model\n");
		// su44: now some models can be NULL, disabled warning
		return;
	}
