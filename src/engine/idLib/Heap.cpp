...
void idHeap::AllocDefragBlock( void ) {
	...
	Com_Printf( "Allocated a %i mb defrag block\n", size / (1024*1024) );
}
...
void idHeap::Free( void *p ) {
	...
	switch( ((byte *)(p))[-1] ) {
		...
		default: {
			Com_FatalError( "idHeap::Free: invalid memory block (%s)", "idLib::sys->GetCallStackCurStr( 4 )" );
			break;
		}
	}
#endif
...
void *idHeap::Allocate16( const dword bytes ) {
	...
	if ( !ptr ) {
		if ( defragBlock ) {
			Com_Printf( "Freeing defragBlock on alloc of %i.\n", bytes );
			free( defragBlock );
			defragBlock = NULL;
			ptr = (byte *) malloc( bytes + 16 + 4 );			
			AllocDefragBlock();
		...
		if ( !ptr ) {
			Com_FatalError( "malloc failure for %i", bytes );
		}
	}
...
dword idHeap::Msize( void *p ) {
	...
	switch( ((byte *)(p))[-1] ) {
		...
		default: {
			Com_FatalError( "idHeap::Msize: invalid memory block (%s)", /*idLib::sys->GetCallStackCurStr( 4 )*/"fixme" );
			return 0;
		}
	}
#endif
...
void idHeap::Dump( void ) {
	...
	for ( pg = smallFirstUsedPage; pg; pg = pg->next ) {
		Com_Printf( "%p  bytes %-8d  (in use by small heap)\n", pg->data, pg->dataSize);
	}

	if ( smallCurPage ) {
		pg = smallCurPage;
		Com_Printf( "%p  bytes %-8d  (small heap active page)\n", pg->data, pg->dataSize );
	}

	for ( pg = mediumFirstUsedPage; pg; pg = pg->next ) {
		Com_Printf( "%p  bytes %-8d  (completely used by medium heap)\n", pg->data, pg->dataSize );
	}

	for ( pg = mediumFirstFreePage; pg; pg = pg->next ) {
		Com_Printf( "%p  bytes %-8d  (partially used by medium heap)\n", pg->data, pg->dataSize );
	}
	
	for ( pg = largeFirstUsedPage; pg; pg = pg->next ) {
		Com_Printf( "%p  bytes %-8d  (fully used by large heap)\n", pg->data, pg->dataSize );
	}

	Com_Printf( "pages allocated : %d\n", pagesAllocated );
}
...
void idHeap::SmallFree( void *ptr ) {
	...
	if ( ix > (256 / ALIGN) ) {
		Com_FatalError( "SmallFree: invalid memory block" );
	}
...
void Mem_DumpCompressed_f( const idCmdArgs &args ) {
	...
	while( arg[0] == '-' ) {
		...
		if ( idStr::Icmp( arg, "s" ) == 0 ) {
			...
			Com_Printf( "memoryDumpCompressed [options] [filename]\n"
						"options:\n"
						"  -s     sort on size\n"
						"  -l     sort on location\n"
...
void Mem_FreeDebugMemory( void *p, const char *fileName, const int lineNumber, const bool align16 ) {
	...
	if ( m->size < 0 ) {
		Com_FatalError( "memory freed twice, first from %s, now from %s", idLib::sys->GetCallStackStr( m->callStack, MAX_CALLSTACK_DEPTH ), idLib::sys->GetCallStackCurStr( MAX_CALLSTACK_DEPTH ) );
	}
