...
void SV_LinkEntity(sharedEntity_t * gEnt) {
	worldSector_t  *node;
	int             leafs[MAX_TOTAL_ENT_LEAFS], cluster, num_leafs, i, j, k, area, lastLeaf;
	float          *origin, *angles;
	svEntity_t     *ent;
...
void SV_ClipToEntity(trace_t * trace, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int entityNum, int contentmask, traceType_t type)
{
	...
	CM_TransformedBoxTrace(trace, (float *)start, (float *)end, (float *)mins, (float *)maxs, clipHandle, contentmask, origin, angles, type);
	if(trace->fraction < 1) {
...
void SV_ClipMoveToEntities(moveclip_t * clip) {
	int             i, num, touchlist[MAX_GENTITIES], passOwnerNum;
	sharedEntity_t *touch;
	trace_t         trace;
	clipHandle_t    clipHandle;
	float          *origin, *angles;
	...
	for(i = 0; i < num; i++) {
		...
		CM_TransformedBoxTrace(&trace, (float *)clip->start, (float *)clip->end, (float *)clip->mins, (float *)clip->maxs, clipHandle, clip->contentmask, origin, angles, clip->collisionType);
		...
		if(trace.fraction < clip->trace.fraction) {
			...
			trace.entityNum = touch->s.number;
			clip->trace = trace;
			clip->trace.startsolid = (qboolean)(( (int)clip->trace.startsolid | (int)oldStart ) != 0);
		}
	}
}
...
int SV_PointContents(const vec3_t p, int passEntityNum) {
	int             touch[MAX_GENTITIES], i, num, contents, c2;
	sharedEntity_t *hit;
	clipHandle_t    clipHandle;
	float          *angles;
