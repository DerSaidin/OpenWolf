...
#define RANDOM_SEED		1013904223L	//((int)Sys_GetClockTicks())
...
void PrintClocks( char *string, int dataCount, int clocks, int otherClocks = 0 ) {
	...
	if ( otherClocks && clocks ) {
		...
		Com_Printf( "c = %4d, clcks = %5d, %d%%\n", dataCount, clocks, p );
	} else {
		Com_Printf( "c = %4d, clcks = %5d\n", dataCount, clocks );
	}
}
...
void TestMemcpy( void ) {
	...
	idRandom rrandom( RANDOM_SEED );

	Com_Printf("====================================\n" );

	for ( i = 5; i < 8192; i += 31 ) {
		for ( j = 0; j < i; j++ ) {
			test0[j] = rrandom.RandomInt( 255 );
		}
		p_simd->Memcpy( test1, test0, 8192 );
		for ( j = 0; j < i; j++ ) {
			if ( test1[j] != test0[j] ) {
				Com_Printf( "   simd->Memcpy() "S_COLOR_RED"X\n" );
				return;
			}
		}
	}
	Com_Printf( "   simd->Memcpy() ok\n" );
}
...
void TestMemset( void ) {
	...
	for ( i = 5; i < 8192; i += 31 ) {
		for ( j = -1; j <= 1; j++ ) {
			...
			for ( k = 0; k < i; k++ ) {
				if ( test[k] != (byte)j ) {
					Com_Printf( "   simd->Memset() "S_COLOR_RED"X\n" );
					return;
				}
			}
		}
	}
	Com_Printf( "   simd->Memset() ok\n" );
}
...
void TestConvertJointMatsToJointQuats( void ) {
	...
	for ( i = 0; i < COUNT; i++ ) {
		if ( !joints1[i].q.Compare( joints2[i].q, 1e-4f ) ) {
			Com_Printf("ConvertJointMatsToJointQuats: broken q %i\n", i );
			break;
		}
		if ( !joints1[i].t.Compare( joints2[i].t, 1e-4f ) ) {
			Com_Printf("ConvertJointMatsToJointQuats: broken t %i\n", i );
			break;
		}
	}
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
...
void TestDeriveTangents( void ) {
	...
	for ( i = 0; i < COUNT; i++ ) {
		...
		if ( !v1.Compare( v2, 1e-1f ) ) {
			Com_Printf("DeriveTangents: broken at normal %i\n -- expecting %s got %s", i, v1.ToString(), v2.ToString());
			break;
		}
		...
		v2.Normalize();
		if ( !v1.Compare( v2, 1e-1f ) ) {
			Com_Printf("DeriveTangents: broken at tangent0 %i -- expecting %s got %s\n", i, v1.ToString(), v2.ToString() );
			break;
		}
		...
		v2.Normalize();
		if ( !v1.Compare( v2, 1e-1f ) ) {
			Com_Printf("DeriveTangents: broken at tangent1 %i -- expecting %s got %s\n", i, v1.ToString(), v2.ToString() );
			break;
		}
		if ( !planes1[i].Compare( planes2[i], 1e-1f, 1e-1f ) ) {
			break;
...
//	idLib::common->SetRefreshOnPrint( true );
	...
	Com_Printf( "using %s for SIMD processing\n", p_simd->GetName() );
