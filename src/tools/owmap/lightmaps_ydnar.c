...
void WriteRGBE(char *filename, float * data, int width, int height)
{
	...
	// write header
	if(fprintf(file,"#?RGBE\n") < 0)
		Error("RGBE write error: %s", filename);

	if(fprintf(file,"# Generated by OWMap \n") < 0)
			Error("RGBE write error: %s", filename);
...
qboolean AddSurfaceToRawLightmap(int num, rawLightmap_t * lm)
{
	...
	if(sampleSize != lm->sampleSize && lmLimitSize == 0)
	{
		Sys_FPrintf(SYS_VRB,
					...
					info->mins[0], info->mins[1], info->mins[2], info->maxs[0], info->maxs[1], info->maxs[2], lm->sampleSize,
					(int)sampleSize);
	}
...
void SetupSurfaceLightmaps(void)
{
	...
	for(i = 0; i < numBSPModels; i++)
	{
		...
		for(j = 0; j < model->numBSPSurfaces; j++)
		{
			...
			if(ds->surfaceType == MST_TRIANGLE_SOUP || ds->surfaceType == MST_FOLIAGE || (info->si->compileFlags & C_VERTEXLIT))
				numSurfsVertexLit++;
			else
			{
				numSurfsLightmapped++;
