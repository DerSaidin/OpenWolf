...
#define MIN_BLOW_VOLUME     30
	...
	while(f)
	{
		...
		if(centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time)
		{
			...
			{
				trap_S_AddLoopingSound( f->ownerCent, f->org, vec3_origin, cgs.media.flameBlowSound);	// JPW NERVE
			}

			if(centFlameStatus[f->ownerCent].streamVolume)
			{
				trap_S_AddLoopingSound( f->ownerCent, f->org, vec3_origin, cgs.media.flameStreamSound);	// JPW NERVE
			}
		...
		for(trav = f; trav; trav = trav->nextFlameChunk)
		{
			...
			{
				trap_S_AddLoopingSound(trav->ownerCent, trav->org, vec3_origin, cgs.media.flameSound);
			}
		}
