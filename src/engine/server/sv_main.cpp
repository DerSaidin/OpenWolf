...
/*
===============
thr_SV_cURL_SendHeartBeat
===============
*/
void *thr_SV_cURL_SendHeartBeat( void *ptr ) {
	CURL *curl;
	CURLcode res;
	char request[1024];
	cvar_t  *net_port;
	...
	if (stricmp(chunk.memory, "banned")) {
		Com_Printf("Heartbeat sent to the official PHP masterserver. %s\n", chunk.memory);
	}
...
*/
void SV_PHPMaster() {
	Com_Printf("Sending heartbeat to the official PHP OpenWolf servers list.\n");
	pthread_create(&svs.thQuery, NULL, thr_SV_cURL_SendHeartBeat, NULL);
}
#endif
...
void SVC_Status(netadr_t from) {
	char            player[1024], status[MAX_MSGLEN], infostring[MAX_INFO_STRING];
