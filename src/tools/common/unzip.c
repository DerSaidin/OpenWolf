	...
	while(pfile_in_zip_read_info->stream.avail_out > 0)
	{
		if((pfile_in_zip_read_info->stream.avail_in == 0) && (pfile_in_zip_read_info->rest_read_compressed > 0))
		{
			...
			if(ZSEEK(pfile_in_zip_read_info->z_filefunc,
					 ...
					 pfile_in_zip_read_info->byte_before_the_zipfile, ZLIB_FILEFUNC_SEEK_SET) != 0)
				return UNZ_ERRNO;
			if(ZREAD(pfile_in_zip_read_info->z_filefunc,
					 pfile_in_zip_read_info->filestream, pfile_in_zip_read_info->read_buffer, uReadThis) != uReadThis)
				return UNZ_ERRNO;
	 ...
	 unsigned len;
{
	...
	if(ZSEEK(pfile_in_zip_read_info->z_filefunc,
			 ...
			 pfile_in_zip_read_info->pos_local_extrafield, ZLIB_FILEFUNC_SEEK_SET) != 0)
		return UNZ_ERRNO;
