...
#if 0
//#if !defined( _WIN32 )
	Com_Printf( "WARNING: IcmpPath used on a case-sensitive filesystem?\n" );
#endif
...
void idStr::Copynz( char *dest, const char *src, int destsize ) {
	if ( !src ) {
		Com_Warning( "idStr::Copynz: NULL src" );
		return;
	}
	if ( destsize < 1 ) {
		Com_Warning( "idStr::Copynz: destsize < 1" ); 
		return;
	}
...
void idStr::Append( char *dest, int size, const char *src ) {
	...
	l1 = strlen( dest );
	if ( l1 >= size ) {
		Com_DropError( "idStr::Append: already overflowed" );
	}
...
int idStr::snPrintf( char *dest, int size, const char *fmt, ...) {
	...
	if ( len >= sizeof( buffer ) ) {
		Com_DropError( "idStr::snPrintf: overflowed buffer" );
	}
	if ( len >= size ) {
		Com_Warning( "idStr::snPrintf: overflow of %i in %i\n", len, size );
		len = size;
	}
...
#ifdef USE_STRING_DATA_ALLOCATOR
	Com_Printf( "%6d KB string memory (%d KB free in %d blocks, %d empty base blocks)\n",
