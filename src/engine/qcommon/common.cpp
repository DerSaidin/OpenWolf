...
void Com_Meminfo_f( void )
{
	...
	int			cryptoBytes, staticBytes, generalBytes;
...
void Com_InitHunkMemory( void )
{
	...
	// bk001205 - was malloc
	s_hunk.original = (byte*)calloc( s_hunk.memSize + 31, 1 );
	if( !s_hunk.original ) {
...
static void Hunk_FrameInit( void )
{
	...
	s_frameStackBase = (byte*)Hunk_Alloc( cb, h_low );
...
void* Hunk_FrameAlloc( size_t cb )
{
	void *ret;
	...
	if(FS_ReadFile("gameinfo.dat", (void **)&f) > 0)
	{
		...
		while((token = COM_Parse(&buf)) != NULL && token[0])
		{
			...
			else if(!Q_stricmp(token, "usesProfiles"))
			{
				if((token = COM_ParseExt(&buf, qfalse)) != NULL && token[0])
				{
					com_gameInfo.usesProfiles = (qboolean)atoi(token);
				}
				else
				{
					FS_FreeFile(f);
...
void Com_Init(char *commandLine)
{
	...
	Cbuf_AddText("exec language.lang\n");	// NERVE - SMF
