...
void CM_PositionTest(traceWork_t * tw) {
	int             leafs[MAX_POSITION_LEAFS], i;
	leafList_t      ll;
...
void CM_TraceThroughSurfaceCollide(traceWork_t * tw, const cSurfaceCollide_t * sc) {
	int             i, j, hit, hitnum;
	float           offset, enterFrac, leaveFrac, t, plane[4] = { 0, 0, 0, 0 }, bestplane[4] = { 0, 0, 0, 0 };
	cPlane_t       *planes;
	cFacet_t       *facet;
	vec3_t          startp, endp;
	static convar_t  *cv;
...
void CM_TraceThroughBrush(traceWork_t * tw, cbrush_t * brush) {
	...
	float           dist, enterFrac, leaveFrac, d1, d2, f, t;
...
static void CM_ProximityToBrush(traceWork_t * tw, cbrush_t * brush) {
	float           dist, minDist = 1e+10f, s, t, sAtMin = 0.0f, radius = 0.0f, fraction;
	int             i;
	cbrushedge_t   *edge;
	traceWork_t     tw2;
...
void CM_TraceThroughSphere(traceWork_t * tw, vec3_t origin, float radius, vec3_t start, vec3_t end) {
	float           l1, l2, length, scale, fraction, a, b, c, d, sqrtd;
...
void CM_TraceThroughVerticalCylinder(traceWork_t * tw, vec3_t origin, float radius, float halfheight, vec3_t start, vec3_t end) {
	float           length, scale, fraction, l1, l2, a, b, c, d, sqrtd;
...
==================
*/
static void CM_Trace(trace_t * results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model, const vec3_t origin, int brushmask, traceType_t type, sphere_t * sphere) {
	int             i;
	traceWork_t     tw;
	vec3_t          offset;
	cmodel_t       *cmod;
...
void CM_TransformedBoxTraceOLD(trace_t * results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model, int brushmask, const vec3_t origin, const vec3_t angles, traceType_t type) {
	...
	vec3_t          start_l, end_l, offset, symetricSize[2], matrix[3], transpose[3];
	trace_t         trace;
	qboolean        rotated;
	int             i;
	sphere_t        sphere;
...
void CM_DrawDebugSurface(void (*drawPoly) (int color, int numPoints, float *points)) {
	...
	cFacet_t                *facet;
	winding_t               *w;
	int                      i, j, k, curplanenum, planenum, curinward, inward;
	float                    plane[4];
