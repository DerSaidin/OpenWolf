...
// 			Cmd_ArgsBuffer(VMA(1), args[2]);
		...
		case CG_R_ADDCORONATOSCENE:
			re.AddCoronaToScene((float*)VMA(1), VMF(2), VMF(3), VMF(4), VMF(5), args[6], (qboolean)args[7]);
			return 0;
		case CG_R_SETFOG:
			re.SetFog(args[1], args[2], args[3], VMF(4), VMF(5), VMF(6), VMF(7));
			return 0;
		case CG_R_SETGLOBALFOG:
			re.SetGlobalFog((qboolean)args[1], args[2], VMF(3), VMF(4), VMF(5), VMF(6));
			return 0;
		...
		case CG_STRNCPY:
			// we cannot return server-address to QVM !
			strncpy((char*)VMA(1), (char*)VMA(2), args[3]);
			return args[1];
		case CG_SIN:
...
qboolean CL_GameConsoleText(void)
{
	...
	return (qboolean)VM_Call(cgvm, CG_CONSOLE_TEXT);
}
