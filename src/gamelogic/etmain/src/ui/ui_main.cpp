...
bool        UI_FeederSelectionClick(itemDef_t * item);
...
static const char *UI_SelectedMap(bool singlePlayer, int index, int *actual);
static int      UI_GetIndexFromSelection(int actual);

static const char *UI_SelectedCampaign(int index, int *actual);
static int      UI_CampaignCount(bool singlePlayer);
...
intptr_t vmMain( intptr_t command, intptr_t arg0, intptr_t arg1, intptr_t arg2, intptr_t arg3, intptr_t arg4, intptr_t arg5, intptr_t arg6, intptr_t arg7, intptr_t arg8, intptr_t arg9, intptr_t arg10, intptr_t arg11, intptr_t arg12, intptr_t arg13, intptr_t arg14, intptr_t arg15 ) {
...
#pragma GCC visibility pop
	...
	switch (command)
	{
		...
		case UI_REPORT_HIGHSCORE_RESPONSE:
			UI_OW_exec(UI_OW_REPORT_HIGHSCORE_RESPONSE, arg0);
			return 0;
		case UI_AUTHORIZED:
			UI_OW_exec(UI_OW_AUTHORIZED, arg0, arg1);
			return 0;
	}
...
char           *Text_AutoWrap_Paint_Chunk(float x, float y, int width, float scale, vec4_t color, char *text, float adjust,
										  int limit, int style, bool dummy, vec4_t color_save)
{
	int             len, count;
	vec4_t          newColor;
	glyphInfo_t    *glyph;
...
int Count_Text_AutoWrap_Paint(float x, float y, int width, float scale, vec4_t color, const char *text, float adjust, int style)
{
	...
	do
	{
		ret = Text_AutoWrap_Paint_Chunk(x, y, width, scale, color, (char *)ret, adjust, 0, style, true, NULL);
		i++;
	} while(ret < end);
						 ...
						 int style)
{
	...
	do
	{
		...
		next = Text_AutoWrap_Paint_Chunk(x, y, width, scale, aux_color, ret, adjust, 0, style, true, next_color);
...
void _UI_Refresh(int realtime)
{
	...
	{
		UI_DrawLoadPanel(true, false, true);
		if(!trap_Cvar_VariableValue("ui_connecting"))
		{
...
bool UI_ParseMenu(const char *menuFile)
{
	int             handle;
	pc_token_t      token;
...
void UI_DrawMapPreview(rectDef_t * rect, float scale, vec4_t color, bool net)
{
...
void UI_DrawNetMapPreview(rectDef_t * rect, float scale, vec4_t color, bool net)
{
...
static void UI_DrawCampaignPreview(rectDef_t * rect, float scale, vec4_t color, bool net)
{
...
static void UI_DrawCampaignCinematic(rectDef_t * rect, float scale, vec4_t color, bool net)
{
	int             campaign = (net) ? ui_currentNetCampaign.integer : ui_currentCampaign.integer;
...
static void UI_DrawCampaignName(rectDef_t * rect, float scale, vec4_t color, int textStyle, bool net)
{
	int             campaign = (net) ? ui_currentNetCampaign.integer : ui_currentCampaign.integer;
...
static void UI_DrawCampaignMapPreview(rectDef_t * rect, float scale, vec4_t color, bool net, int map)
{
	int             campaign = (net) ? ui_currentNetCampaign.integer : ui_currentCampaign.integer;
...
//          Q_CleanStr( namebuf );
			...
			if(muted)
			{
				uiInfo.playerMuted[uiInfo.playerCount] = true;
			}
			else
			{
				uiInfo.playerMuted[uiInfo.playerCount] = false;
			}
...
/*		case UI_JOINGAMETYPE:
		...
		case UI_NETMAPPREVIEW:
			UI_DrawNetMapPreview(&rect, scale, color, true);
			break;
/*		case UI_MAP_TIMETOBEAT:
		...
		case UI_CAMPAIGNCINEMATIC:
			UI_DrawCampaignCinematic(&rect, scale, color, false);
			break;
		case UI_CAMPAIGNNAME:
			UI_DrawCampaignName(&rect, scale, color, textStyle, false);
			break;
		case UI_CAMPAIGNDESCRIPTION:
			UI_DrawCampaignDescription(&rect, scale, color, text_x, text_y, textStyle, align, true);
			break;
		case UI_GAMETYPEDESCRIPTION:
			UI_DrawGametypeDescription(&rect, scale, color, text_x, text_y, textStyle, align, true);
			break;
		...
		case UI_CAMPAIGNMAP6_SHOT:
			UI_DrawCampaignMapPreview(&rect, scale, color, false, ownerDraw - UI_CAMPAIGNMAP1_SHOT);
			break;
		...
		case UI_CAMPAIGNMAP6_TEXT:
			UI_DrawCampaignMapDescription(&rect, scale, color, text_x, text_y, textStyle, align, false,
...
/*		case UI_BOTSKILL:
		...
		case UI_KEYBINDSTATUS:
			...
			break;
		case UI_LOADPANEL:
			UI_DrawLoadPanel(false, true, false);
			break;
		default:
			break;
	}
...
void UI_RunMenuScript(char **args)
{
	...
	if(String_Parse(args, &name)){
		...
		} else if( Q_stricmp( name, "resetlogin" ) == 0 ) {
...
//          UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);
			...
			UI_StartServerRefresh(uiInfo.serverStatus.numDisplayServers ? false : true);	// if we don't have any valid servers, it's kinda safe to assume we would like to get a full new list
			UI_BuildServerDisplayList(true);
		}
		...
		else if(Q_stricmp(name, "Leave") == 0)
		{
			...
			else
			{
				...
				trap_Key_SetCatcher(KEYCATCH_UI);
				Menus_CloseAll();
//				Menus_ActivateByName( "background_1", true );
				Menus_ActivateByName("backgroundmusic", true);
				Menus_ActivateByName("main_opener", true);
			}
		}
		...
		else if(Q_stricmp(name, "reconnect") == 0)
		{
			...
			if(ui_mapIndex.integer >= UI_MapCountByGameType(false))
			{
				Menu_SetFeederSelection(NULL, FEEDER_MAPS, 0, NULL);
				Menu_SetFeederSelection(NULL, FEEDER_ALLMAPS, 0, NULL);
			}
...
*/
#if 0							// rain - unused
static int UI_MapCountByCampaign(bool singlePlayer)
{
	int             campaign;
	int             i, count = 0;
...
static int UI_FeederCount(float feederID)
{
	...
	{
		return UI_CampaignCount(feederID == FEEDER_CAMPAIGNS ? true : false);
	}
...
const char     *UI_FeederItemText(float feederID, int index, int column, qhandle_t * handles, int *numhandles)
{
	...
	*numhandles = 0;
	...
	else if(feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS)
	{
		...
		return UI_SelectedMap(feederID == FEEDER_MAPS ? true : false, index, &actual);
	}
	else if(feederID == FEEDER_CAMPAIGNS || feederID == FEEDER_ALLCAMPAIGNS)
	{
		int             actual;
...
static qhandle_t UI_FeederItemImage(float feederID, int index)
{
	...
	else if(feederID == FEEDER_ALLMAPS || feederID == FEEDER_MAPS)
	{
		...
		UI_SelectedMap(feederID == FEEDER_MAPS ? true : false, index, &actual);
		index = actual;
...
extern void     Item_ListBox_MouseEnter(itemDef_t * item, float x, float y, bool click);
bool UI_FeederSelectionClick(itemDef_t * item)
{
	...
	if(item->special == FEEDER_SERVERS && !Menus_CaptureFuncActive())
	{
		...
		Item_ListBox_MouseEnter(item, DC->cursorx, DC->cursory, true);
  ...
  while ( 1 ) {
	...
	if (token[0] == '{') {
	  ...
	  if (tempStr && (tempStr[0] == 'f' || tempStr[0] == 'F')) {
		uiInfo.characterList[uiInfo.characterCount].female = true;
	  } else {
		uiInfo.characterList[uiInfo.characterCount].female = false;
	  }
...
static bool MapList_Parse(char **p)
{
	...
	while(1)
	{
		...
		if(token[0] == '{')
		{
			...
			//  uiInfo.mapList[uiInfo.mapCount].cinematic = trap_CIN_PlayCinematic(va("%s.roq",uiInfo.mapList[uiInfo.mapCount].mapLoadName), false, false, true, 0, 0, 0, 0);
			//}
...
void _UI_Init(bool inGameLoad)
{
	...
	UI_LoadMenus("ui/menus.txt", false);
	...
	if(Menu_Count() > 0)
	{
		...
		switch (menu)
		{
			...
			case UIMENU_MAIN:
				...
				Menus_CloseAll();
				//Menus_ActivateByName( "background_1", true );
				Menus_ActivateByName("backgroundmusic", true);	// Arnout: not nice, but best way to do it - putting the music in it's own menudef
				...
				if((*buf) && (Q_stricmp(buf, ";")))
				{
					...
					if(!Q_stricmpn(buf, "Invalid password", 16))
					{
						...
						Menus_ActivateByName("popupPassword", true);
					}
					...
					{			// fretn
						...
						switch (ui_autoredirect.integer)
						{
							...
							default:
								...
								Menus_ActivateByName("popupServerRedirect", true);
								break;
						}
					}
					else
					{
						...
						if(pb_enable)
						{
							Menus_ActivateByName("popupError_pbenable", true);
						}
						else
						{
							Menus_ActivateByName("popupError", true);
						}
					}
				}

				trap_S_FadeAllSound(1.0f, 1000, false);	// make sure sound fades up
			...
			case UIMENU_INGAME:
				...
				Menus_ActivateByName("ingame_main", true);
				return;
...
//  static bool playingMusic = false;
	...
	if(!overlay)
	{
		UI_DrawLoadPanel(false, false, false);
	}
	else
	{
//      if( !playingMusic ) {
...
int QDECL UI_OW_exec( int cmd, ... )
{
	va_list		argptr;
	va_start(argptr, cmd);

	switch ( cmd ) {
		case UI_OW_REQUEST_HIGHSCORES: {
				trap_Cmd_ExecuteText( EXEC_APPEND, va( "globalhighscores \"%s\";", UI_Cvar_VariableString("name") ) );
			}
			break;
		case UI_OW_REPORT_HIGHSCORE_RESPONSE:
			{
				// \next_points\220\percentile\0.196723144834655\next_name\^2FrZ|^3Blaze\rank\3\points\0
				if ( trap_Cvar_VariableInt( "cl_gamename" ) == 0 ) {
					char info[ MAX_INFO_STRING ];

					trap_Cvar_VariableStringBuffer( "ow_postresults", info, sizeof(info) );
				}
			} break;
		case UI_OW_AUTHORIZED:
			{
