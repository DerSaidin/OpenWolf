...
static void CG_EntityEffects(centity_t * cent)
{
	...
	if(cent->currentState.loopSound)
	{
		...
		else if(cent->currentState.solid == SOLID_BMODEL)
		{
			...
			v = cgs.inlineModelMidpoints[cent->currentState.modelindex];
			VectorAdd(cent->lerpOrigin, v, origin);
			trap_S_AddLoopingSound(cent->currentState.number, origin, vec3_origin, cgs.gameSounds[cent->currentState.loopSound]);
		}
		else
		{
			trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.gameSounds[cent->currentState.loopSound]);
	...
	if(CG_EntOnFire(cent))
	{
		...
		trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.flameBlowSound);
		// play a burning sound when not moving
		trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.flameSound);
	}
...
//----(SA)  whoops, didn't mean to check it in with the missile flare
	...
	if(cent->currentState.weapon == WP_DYNAMITE)
	{
		if(cent->currentState.teamNum < 4)
		{
			...
			BG_EvaluateTrajectoryDelta(&cent->currentState.pos, cg.time, velocity, qfalse, -1);
			trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, velocity, weapon->spindownSound);
		}
	}
	...
	if(s1->modelindex2)
	{
		if(cent->currentState.powerups == STATE_UNDERCONSTRUCTION)
		{
			...
			switch( cent->currentState.frame ) {
			   case 1: trap_S_AddLoopingSound( cent->currentState.number, cent->currentState.origin2, vec3_origin, cgs.media.buildSound[0] ); break;
			   case 2: trap_S_AddLoopingSound( cent->currentState.number, cent->currentState.origin2, vec3_origin, cgs.media.buildSound[1] ); break;
			   case 3: trap_S_AddLoopingSound( cent->currentState.number, cent->currentState.origin2, vec3_origin, cgs.media.buildSound[2] ); break;
			   case 4: trap_S_AddLoopingSound( cent->currentState.number, cent->currentState.origin2, vec3_origin, cgs.media.buildSound[3] ); break;
			   }
		}
