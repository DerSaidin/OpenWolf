...
success:
	...
	// SDL_SetGamma will incorrectly return qfalse... the first time; ask
...
void GLimp_EndFrame( void )
{
	...
	if ( r_fullscreen->modified )
	{
		...
		if ( s )
		{
			...
			fullscreen = (qboolean)!!( s->flags & SDL_FULLSCREEN );
			...
			needToToggle = (qboolean)(!!r_fullscreen->integer != fullscreen);

			if ( needToToggle )
			{
				sdlToggled = (qboolean)SDL_WM_ToggleFullScreen( s );
			}
		}
