...
void idCollisionModelManagerLocal::SetupTrmModelStructure( void ) {
	...
	trmMaterial = declManager_FindMaterial( "_tracemodel", false );
	if ( !trmMaterial ) {
		Com_Printf( "_tracemodel material not found\n" );
	}
...
================
*/
cmHandle_t idCollisionModelManagerLocal::SetupTrmModel( const idTraceModel &trm, idMaterial *material ) {
	int i, j;
	cm_vertex_t *vertex;
	cm_edge_t *edge;
	cm_polygon_t *poly;
			...
			for ( planeNum = startPlane, i = 0; i < b->numPlanes; i++, planeNum++ ) {
				...
				if ( res == SIDE_BACK ) {
					if ( cm_outList->numWindings >= MAX_WINDING_LIST ) {
						Com_Warning( "idCollisionModelManagerLocal::ChopWindingWithBrush: primitive %d more than %d windings", list->primitiveNum, MAX_WINDING_LIST );
						return;
					}
...
================
*/
void idCollisionModelManagerLocal::CreatePolygon( cm_model_t *model, idFixedWinding *w, const idPlane &plane, idMaterial *material, int primitiveNum ) {
	...
	if ( numPolyEdges > CM_MAX_POLYGON_EDGES ) {
		Com_Warning( "idCollisionModelManagerLocal::CreatePolygon: polygon has more than %d edges", numPolyEdges );
...
================
*/
void idCollisionModelManagerLocal::PolygonFromWinding( cm_model_t *model, idFixedWinding *w, const idPlane &plane, idMaterial *material, int primitiveNum ) {
	int contents;
	...
	if ( w->IsHuge() ) {
		Com_Warning( "idCollisionModelManagerLocal::PolygonFromWinding: model %s primitive %d is degenerate", model->name.c_str(), abs(primitiveNum) );
		return;
	}
...
=================
*/
void idCollisionModelManagerLocal::CreatePatchPolygons( cm_model_t *model, idSurface_Patch &mesh, idMaterial *material, int primitiveNum ) {
	int i, j;
	float dot;
	int v1, v2, v3, v4;
	idFixedWinding w;
...
void idCollisionModelManagerLocal::ConvertPatch( cm_model_t *model, const idMapPatch *patch, int primitiveNum ) {
	...
	material = declManager_FindMaterial( patch->GetMaterial() );
#ifdef CONTENTS_REMOVE_UTIL
	if ( !( material->GetContentFlags() & CONTENTS_REMOVE_UTIL ) ) {
		return;
	}
#endif // CONTENTS_REMOVE_UTIL
...
void idCollisionModelManagerLocal::ConvertBrushSides( cm_model_t *model, const idMapBrush *mapBrush, int primitiveNum ) {
	...
	for ( i = 0; i < mapBrush->GetNumSides(); i++ ) {
		...
		material = declManager_FindMaterial( mapSide->GetMaterial() );
#ifdef CONTENTS_REMOVE_UTIL
		if ( !( material->GetContentFlags() & CONTENTS_REMOVE_UTIL ) ) {
			continue;
		}
...
cm_model_t *idCollisionModelManagerLocal::LoadRenderModel( const char *fileName ) {
	...
	Com_Printf( "loaded collision model %s\n", model->name.c_str() );
...
void idCollisionModelManagerLocal::ModelInfo( cmHandle_t model ) {
	...
	if ( model < 0 || model > MAX_SUBMODELS || model > maxModels ) {
		Com_Printf( "idCollisionModelManagerLocal::ModelInfo: invalid model handle\n" );
		return;
	}
	if ( !models[model] ) {
		Com_Printf( "idCollisionModelManagerLocal::ModelInfo: invalid model\n" );
		return;
	}
...
void idCollisionModelManagerLocal::LoadMap( const idMapFile *mapFile ) {
	...
	if ( mapFile == NULL ) {
		Com_FatalError( "idCollisionModelManagerLocal::LoadMap: NULL mapFile" );
	}
...
const char *idCollisionModelManagerLocal::GetModelName( cmHandle_t model ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelBounds: invalid model handle\n" );
		return "";
	}
...
bool idCollisionModelManagerLocal::GetModelContents( cmHandle_t model, int &contents ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelContents: invalid model handle\n" );
		return false;
	}
...
bool idCollisionModelManagerLocal::GetModelVertex( cmHandle_t model, int vertexNum, idVec3 &vertex ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelVertex: invalid model handle\n" );
		return false;
	}

	if ( vertexNum < 0 || vertexNum >= models[model]->numVertices ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelVertex: invalid vertex number\n" );
		return false;
	}
...
bool idCollisionModelManagerLocal::GetModelEdge( cmHandle_t model, int edgeNum, idVec3 &start, idVec3 &end ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelEdge: invalid model handle\n" );
		return false;
	}

	edgeNum = abs( edgeNum );
	if ( edgeNum >= models[model]->numEdges ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelEdge: invalid edge number\n" );
		return false;
	}
...
bool idCollisionModelManagerLocal::GetModelPolygon( cmHandle_t model, int polygonNum, idFixedWinding &winding ) const {
	...
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		Com_Printf( "idCollisionModelManagerLocal::GetModelPolygon: invalid model handle\n" );
		return false;
	}
...
cmHandle_t idCollisionModelManagerLocal::LoadModel( const char *modelName, const bool precache ) {
	...
	if ( numModels >= MAX_SUBMODELS ) {
		Com_FatalError( "idCollisionModelManagerLocal::LoadModel: no free slots\n" );
		return 0;
	}
	...
	if ( LoadCollisionModelFile( modelName, 0 ) ) {
		...
		if ( handle >= 0 ) {
			...
			Com_Warning( "idCollisionModelManagerLocal::LoadModel: collision file for '%s' contains different model", modelName );
		}
	}
...
bool idCollisionModelManagerLocal::TrmFromModel( const cm_model_t *model, idTraceModel &trm ) {
	...
	if ( model->numVertices > MAX_TRACEMODEL_VERTS ) {
		Com_Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many vertices.\n", model->name.c_str() );
		PrintModelInfo( model );
		return false;
	}
	...
	if ( model->numEdges > MAX_TRACEMODEL_EDGES+1 ) {
		Com_Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many edges.\n", model->name.c_str() );
		PrintModelInfo( model );
		return false;
	}
	...
	if ( !TrmFromModel_r( trm, model->node ) ) {
		Com_Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many polygons.\n", model->name.c_str() );
		PrintModelInfo( model );
		return false;
	}
	...
	for ( i = 1; i <= trm.numEdges; i++ ) {
		if ( numEdgeUsers[i] != 2 ) {
			Com_Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has dangling edges, the model has to be an enclosed hull.\n", model->name.c_str() );
			PrintModelInfo( model );
			return false;
		}
	}
...
bool idCollisionModelManagerLocal::TrmFromModel( const char *modelName, idTraceModel &trm ) {
	...
	if ( !handle ) {
		Com_Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s not found.\n", modelName );
		return false;
	}
...
idMaterial *idCollisionModelManagerLocal::declManager_FindMaterial(const char *name, bool smth) {
	static idMaterial tmp;
	return &tmp;
}
