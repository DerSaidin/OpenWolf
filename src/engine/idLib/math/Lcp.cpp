...
void idLCP_Square::RemoveClamped( int r ) {
	...
	for ( i = r; i < numClamped; i++ ) {
		...
		if ( diag == 0.0f ) {
			Com_Printf( "idLCP_Square::RemoveClamped: updating factorization failed\n" );
			return;
		}
...
bool idLCP_Square::Solve( const idMatX &o_m, idVecX &o_x, const idVecX &o_b, const idVecX &o_lo, const idVecX &o_hi, const int *o_boxIndex ) {
	...
	if ( numUnbounded ) {
		...
		if ( !FactorClamped() ) {
			Com_Printf( "idLCP_Square::Solve: unbounded factorization failed\n" );
			return false;
		}
	...
	if ( numIgnored ) {
		if ( lcp_showFailures ) {
			Com_Printf( "idLCP_Symmetric::Solve: %d of %d bounded variables ignored\n", numIgnored, m.GetNumRows() - numUnbounded );
		}
	}
#endif
	...
	if ( failed ) {
		if ( lcp_showFailures ) {
			Com_Printf( "idLCP_Square::Solve: %s (%d of %d bounded variables ignored)\n", failed, m.GetNumRows() - i, m.GetNumRows() - numUnbounded );
		}
...
void idLCP_Symmetric::AddClamped( int r, bool useSolveCache ) {
	...
	if ( d == 0.0f ) {
		Com_Printf( "idLCP_Symmetric::AddClamped: updating factorization failed\n" );
		numClamped++;
		return;
	}
...
void idLCP_Symmetric::RemoveClamped( int r ) {
	...
	if ( r == 0 ) {
		...
		if ( numClamped == 1 ) {
			diag = rowPtrs[0][0];
			if ( diag == 0.0f ) {
				Com_Printf( "idLCP_Symmetric::RemoveClamped: updating factorization failed\n" );
				return;
			}
			clamped[0][0] = diag;
...
bool idLCP_Symmetric::Solve( const idMatX &o_m, idVecX &o_x, const idVecX &o_b, const idVecX &o_lo, const idVecX &o_hi, const int *o_boxIndex ) {
	...
	if ( numUnbounded ) {
		...
		// factor and solve for unbounded variables
		if ( !FactorClamped() ) {
			Com_Printf( "idLCP_Symmetric::Solve: unbounded factorization failed\n" );
			return false;
		}
		SolveClamped( f, b.ToFloatPtr() );
	...
	if ( failed ) {
		if ( lcp_showFailures ) {
			Com_Printf( "idLCP_Symmetric::Solve: %s (%d of %d bounded variables ignored)\n", failed, m.GetNumRows() - i, m.GetNumRows() - numUnbounded );
		}
		for ( j = i; j < m.GetNumRows(); j++ ) {
			f[j] = 0.0f;
		}
