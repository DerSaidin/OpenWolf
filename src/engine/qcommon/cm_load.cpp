...
clipMap_t       cm;
int             c_pointcontents, c_traces, c_brush_traces, c_patch_traces, c_trisoup_traces;
...
static void CMod_CreateBrushSideWindings(void) {
	int             i, j, k, numEdges, edgesAlloc, totalEdgesAlloc = 0, totalEdges = 0;
	winding_t      *w;
...
void CMod_LoadEntityString(lump_t * l) {
	char *p, *token, keyname[MAX_TOKEN_CHARS], value[MAX_TOKEN_CHARS];
...
void CMod_LoadSurfaces(lump_t * surfs, lump_t * verts, lump_t * indexesLump) {
	drawVert_t     *dv, *dv_p;
	dsurface_t     *in;
	int             count, i, j, numVertexes, width, height, shaderNum, numIndexes, *index, *index_p;
	cSurface_t     *surface;
	...
	for(i = 0; i < count; i++, in++) {
		if(LittleLong(in->surfaceType) == MST_PLANAR) {
#ifdef USE_PHYSICS
			int j = 0, modelIndex = CMod_PhysicsBSPSurfaceIsModel(i);
			...
			int j = 0, k = 0, rowLimit = in->patchHeight - 1, colLimit = in->patchWidth - 1;
...
void CM_LoadMapOLD(const char *name, qboolean clientload, int *checksum) {
	...
	*checksum = last_checksum;
	...
	if ( header.version != BSP_VERSION && header.version != BSP_VERSION_Q3 ) {
		Com_Error (ERR_DROP, "CM_LoadMap: %s has wrong version number (%i should be %i for ET or %i for Q3)", name, header.version, BSP_VERSION, BSP_VERSION_Q3 );
	}
