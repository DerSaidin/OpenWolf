...
void BotDrawDebugPolygons(BotPolyFunc drawPoly, int value) {
	...
	for(i = 0; i < MAX_DEBUGPOLYS; i++) {
		...
		Com_DPrintf("poly %i, numpoints = %d\n", i, poly->numPoints);
	}
}
...
qboolean BotImport_BotVisibleFromPos(vec3_t srcorigin, int srcnum, vec3_t destorigin, int destent, qboolean dummy) {
	return (qboolean)VM_Call(gvm, BOT_VISIBLEFROMPOS, srcorigin, srcnum, destorigin, destent, dummy);
}
...
qboolean BotImport_BotCheckAttackAtPos(int entnum, int enemy, vec3_t pos, qboolean ducking, qboolean allowHitWorld) {
	return (qboolean)VM_Call(gvm, BOT_CHECKATTACKATPOS, entnum, enemy, pos, ducking, allowHitWorld);
}
...
// added single player
	return botlib_export->BotLibSetup((qboolean)(SV_GameIsSinglePlayer() || SV_GameIsCoop()));
